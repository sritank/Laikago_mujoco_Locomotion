...
class LaikagoEnv2(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
        mujoco_env.MujocoEnv.__init__(self, 'laikago2.xml', 5)
        utils.EzPickle.__init__(self)
        ...
        # low_obs = np.ones(28)*-10
        # high_obs = np.ones(28)*10
    ...
    def step(self, action):
        ...
        action=1.5*action+7
        action2 = np.array([-2, action[0], -2, action[0], -2, action[1], -2, action[1]])
        action=action2
