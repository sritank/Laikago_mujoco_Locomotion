...
class LaikagoEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
        ...
        self.action_space = spaces.Box(low=np.ones(8)*-2, high=np.ones(8)*2, dtype=np.float16)
        ...
        low_obs = np.ones(94)*-1
        high_obs = np.ones(94)*1
    ...
    def step(self, action):
        ...
        reward_ctrl =  -0*1e-3  * np.square(action).sum()
        ...
        reward_dist = 1*10*(self.sim.data.qpos[0] - self.init_qpos[0])
        # reward_ht = -1*1000*(self.sim.data.qpos[2] + 0.08244029956850837)**2
        reward_ht = 0/(self.sim.data.qpos[2] + 0.08244029956850837)**2
        reward_eplength = 0*self.sim.data.time
        # print("qpos")
        ...
        if np.abs(self.sim.data.sensordata[61])<0.94:
            done=True
            # reward = -100
    ...
    def reset(self):
        ...
        qposlimit=15; qvellimit=4; anglelimit = 6.28; omegalimit=10; vellimit=10; acclimit=20; flimit=100000; 
